 ...
 *
 * NOTE: V3 analyzes the visible range specified by zoomRange parameter.
 * This range is locked when V3 is first enabled and does NOT change on zoom.
 * Signals will only appear within the original locked range.
 */
...
export const calculateVolumeProfileV3PL = ({
  ...
  let allTimeHigh = 0 // Track all-time high price for current holding period (reset on each buy/sell)
  ...
  for (let i = 0; i < reversedPrices.length; i++) {
    ...
    // Update all-time high ONLY while holding
    // Each holding period tracks its own ATH for window reset purposes
    ...
    if (breakSignal) {
      if (breakSignal.isUpBreak) {
        ...
        if (isHolding && pointsSinceWindowReset >= MIN_POINTS_FOR_SELL) {
          ...
          currentWindowIndex = null
          pointsSinceWindowReset = 0
          allTimeHigh = 0 // Reset ATH for next holding period
          currentTradeId++
        }
